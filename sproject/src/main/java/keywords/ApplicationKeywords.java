...
public class ApplicationKeywords extends ValidationKeywords {
	...
	public int findCurrentStockQuantity(String companyName) {
		//to find stock quantity of given stock
		int rowNum = getRowNumWithCellData("stockTable_id", companyName);
		
		if (rowNum == -1) {
			logInfo("Stock Quantity is 0 as given Stock - " + companyName + " is not present in Stock List");
			return 0;
		}
	...
	public void openTrasactionHistory(String companyName) {
		//it will click on the transaction history of that particular company
		int rowNum = getRowNumWithCellData("stockTable_id", companyName);
	...
	public int getRowNumWithCellData(String locatorKey, String data) {
	    ...
	    for (int rNum = 0; rNum < rows.size(); rNum++) {
	        ...
	        for (int cNum = 0; cNum < cells.size(); cNum++) {
	            ...
	            // Only print cell data if it is not empty
	            if (!cell.getText().trim().equals("")) {
	                //System.out.println("Cell Data :: " + cell.getText());
	                
	                // Check if the cell contains the data, ignoring case
	                if (cell.getText().toLowerCase().contains(data.toLowerCase())) {
	                    return rNum; // Return row number when match is found
	                }
	            }
	        }
	    }
	...
	public int getRowNumWithCellDataOld(String locatorKey, String data) {
